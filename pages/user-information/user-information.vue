<template>
	<view>
		<cu-custom bgColor="bg-gradual-tab" :isBack="true">
			...
			<block slot="content" style="font-size: 28rpx!important; letter-spacing: 1rpx;">发布历史</block>
		</cu-custom>
		 ...
		 :current="TabCur" @change="tabSwiper">
			<swiper-item class="animation-fade" v-if="!loading">
				<scroll-view scroll-y :style="{height:scroll_height +'px'}">
					<image src="../../static/article/no-history.png" class="none-default-image" mode="widthFix" v-if="!loading&&articleInfo.length==0"></image>
			...
			<swiper-item class="animation-fade" v-if="!loading">
				<scroll-view scroll-y :style="{height:scroll_height +'px'}">
					<image src="../../static/article/no-history.png" class="none-default-image" mode="widthFix" v-if="!loading&&activityInfo.length==0"></image>
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			this.TabCur = option.current_tab;
			this.loading = true;
...
<style src="./user-information.css">
</style>

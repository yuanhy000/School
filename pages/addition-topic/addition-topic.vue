<template>
	<view class="addition-container" @tap="cancleTextareaInput">
		<cu-custom bgColor="bg-gradual-tab" :isBack="true">
			...
			<block slot="content" style="font-size: 28rpx!important; letter-spacing: 1rpx;">发布话题</block>
		</cu-custom>
		<scroll-view scroll-y id="scroll" :style="{height:scroll_height +'px'}">
			<view class="padding-top">
				<view class="cu-tabbar-height bg-white margin-left margin-right  border-radius bg-white shadow flex align-center ">
					<input class="margin-left margin-right text-sm max-width" type="text" placeholder="输入你的问题" v-model="title" />
				</view>
				<view class="cu-form-group margin-left margin-top margin-right margin-bottom border-radius bg-white shadow">
					<textarea class="textarea-font-size" placeholder="对问题进行补充说明..." v-model="content" maxlength="400" v-if="displayTextarea"
...
<script>
	...
	export default {
		...
		methods: {
			...
			Submit() {
				if(this.title == ''){
					this.toastContent = '问题不能为空'
					this.showToast = true;
					return ;
				}
				if(this.content == ''){
					this.toastContent = '话题内容不能为空'
					this.showToast = true;
					return ;
				}

<template>
	<view>
		<cu-custom bgColor="bg-gradual-tab" :isBack="true">
			...
			<block slot="content" style="font-size: 28rpx!important; letter-spacing: 1rpx;">招募详情</block>
		</cu-custom>
		...
		<view class="cu-bar padding-right bg-white info-border-top animation-fade-quick max-width" v-show="display" style="position: fixed;"
		 :style="{bottom: fixedHeight+'px'}">
			<view class="flex">
				<view class="action flex-column align-center padding-right-xl icon-button" v-if="!isInput" @click="likeRecruit">
				...
				<view class="action flex-column align-center padding-right-xl icon-button" v-if="!isInput" @click="CollectRecruit">
			...
			formatTime() {
				...
				if (differenceDay === 0) {
					...
					if (differenceWeekDay > 0) {
						...
						this.recruitInfo.display_time = this.recruitInfo.recruit_created;
					}
				}
			},
			formatCommentsTime(index) {
				...
				if (differenceDay === 0) {
					...
					if (differenceWeekDay > 0) {
						...
						this.recruitInfo.recruit_comments[index].display_time = this.recruitInfo.recruit_comments[index].comment_created;
					}
				}
			}
		}

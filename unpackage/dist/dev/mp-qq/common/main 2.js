...
//
//
var _default = { data: function data() {return { displayCategory: false, categoryList: [], currentCategory: {}, categoryName: '' };}, mounted: function mounted() {var _this = this;_vue.default.prototype.$http.get('/categories/get').then(function (res) {console.log(res);_this.categoryList = res.data.data.categories;});}, methods: { ChangeCategory: function ChangeCategory(index) {this.currentCategory = this.categoryList[index];this.categoryName = this.currentCategory.category_name;this.hideModal();}, showModal: function showModal(e) {this.displayCategory = true;}, hideModal: function hideModal(e) {this.displayCategory = false;}, tabSelect: function tabSelect(e) {this.TabCur = e.currentTarget.dataset.id;this.scrollLeft = (e.currentTarget.dataset.id - 1) * 60;} } };exports.default = _default;
